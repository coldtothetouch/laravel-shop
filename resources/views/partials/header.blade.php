        ...
        <div class="header-inner flex items-center justify-between lg:justify-start">
            ...
            <div class="header-menu grow hidden lg:flex items-center ml-8 mr-8 gap-8">
                ...
                <nav class="hidden 2xl:flex gap-8">
                    ...
                    <a href="{{--{{ route('cart') }}--}}" class="text-white hover:text-pink font-bold">Корзина</a>
                </nav>
            </div><!-- /.header-menu -->
            <div class="header-actions flex items-center gap-3 md:gap-5">
                @auth()
                    <div x-data="{dropdownProfile: false}" class="profile relative">
                        ...
                        >
                            ...
                            <div class="mt-4">
                                <ul class="space-y-2">
                                    <li><a href="{{--{{ route('orders') }}--}}" class="text-body hover:text-white text-xs font-medium">Мои заказы</a></li>
                                    <li><a href="{{--{{ route('profile') }}--}}" class="text-body hover:text-white text-xs font-medium">Редактировать профиль</a></li>
                                </ul>
                            </div>
                            <div class="mt-6">
                                <a href="#" class="inline-flex items-center text-body hover:text-pink">
                                    ...
                                    <form action="{{ route('auth.logout') }}" method="POST" class="ml-2 font-medium">
                                        @csrf
                                        @method('DELETE')
                ...
                @elseguest()
                    <a href="{{ route('auth.login.index') }}" class="profile hidden xs:flex items-center">
